...
export function loadConfig(): ScannerConfig {
  return {
    ...
    maxPagesPerQuery: parseInt(process.env.SCAN_MAX_PAGES || '1', 10),
    searchQueries: (process.env.SCAN_QUERIES || 'iphone 13,iphone 14,iphone 15,iphone 16').split(',').map(s => s.trim()),
